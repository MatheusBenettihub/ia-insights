...
def extract_price_target_from_query(query):
    ...
    # Padrões: $65k, 65k, $65000, 65000, $98k
    import re as _re
    # Formato com 'k' — ex: 65k, $65k
    m = _re.search(r'\$?(\d{2,3})k', q)
    if m:
        return float(m.group(1)) * 1000
    # Formato numérico longo — ex: $65000, 100000
    m = _re.search(r'\$?(\d{5,7})\b', q)
    if m:
...
def get_sp500_data():
    """Busca histórico completo do S&P500 (cache 24h — dados diários)."""
    closes, timestamps, dates = fetch_sp500_history()
...
def build_prompt(ind, macro, deriv, historical_candle=None, level_target=None, sp500_ctx="", pattern_ctx=""):
...
REGRA: Use esses valores exatos ao falar sobre essa data. Nunca estime."""
    ...
    # Level breakout probability (se pergunta mencionar preço-alvo)
    level_ctx = ""
    if level_target and ind.get("_closes"):
        lbp = level_breakout_probability(ind["_closes"], level_target)
        if lbp:
            level_ctx = f"""
PROBABILIDADE DE ROMPIMENTO — NÍVEL ${lbp['alvo']:,.0f} ({lbp['direcao']}):
  Distância do preço atual: {lbp['distancia_pct']:+.1f}%
  {lbp['resumo']}
  Taxa de hit em 14 dias: {lbp['taxa_hit_14d']}%
  Taxa de hit em 30 dias: {lbp['taxa_hit_30d']}%
  Amostras históricas: {lbp['amostras']}
...
12. Proibido afirmar contexto qualitativo (ETFs, regulação, institucionais) sem dado real disponível
...
def send_message(api_key, user_msg, ind, macro, deriv, historical_candle=None, level_target=None, sp500_ctx="", pattern_ctx=""):
    try:
...
with st.spinner("Carregando dados históricos..."):
    ...
    sp500_closes, sp500_timestamps = get_sp500_data()
...
if final_prompt:
    ...
    # Detecta nível de preço-alvo na pergunta
    level_target = extract_price_target_from_query(final_prompt)

    # Monta contexto cross-market S&P500 (fingerprint BTC atual → similar structure no S&P500)
    sp500_ctx = ""
    if sp500_closes and ind and "error" not in ind:
        try:
            closes_btc = ind.get("_closes", [])
            if len(closes_btc) >= 200:
                from btc_analytics import _ema_series, _rsi_series
                ema50_btc  = _ema_series(closes_btc, 50)[-1]
                ema200_btc = _ema_series(closes_btc, 200)[-1]
                rsi_btc    = _rsi_series(closes_btc[-50:], 14)[-1]
                price_btc  = closes_btc[-1]
                rets30     = [(closes_btc[i]-closes_btc[i-1])/closes_btc[i-1]*100 for i in range(len(closes_btc)-30, len(closes_btc))]
                vol30_btc  = (sum(r**2 for r in rets30)/30)**0.5
    ...
    try:
        ...
        if len(btc_closes_raw) >= 180:
            pattern_ctx = build_pattern_context(
                ...
                sp500_closes if sp500_closes else None,
    ...
    with st.chat_message("assistant"):
        with st.spinner("Analisando..."):
            reply, err = send_message(api_key, final_prompt, ind, macro, deriv, historical_candle, level_target, sp500_ctx, pattern_ctx)
        if err:
            st.error(err)
        else:
            st.markdown(reply)
